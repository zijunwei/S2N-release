        ...
        for video_idx, (s_name, s_feature, s_groundtruth, s_segments) in enumerate(zip(self.videonames, self.videofeatures, self.groundtruthscores, self.segments)):
            ...
            for s_sample_rate in self.sample_rate:
                ...
                for s_proposed_segment in proposedSegments:
                    ...
                    _, head_positions, _, tail_positions, cls_scores, _ = model(s_feature)
                    head_positions, tail_positions = helper.reorder(head_positions, tail_positions)
                    head_positions = (head_positions * s_sample_rate + startIdx)
                    tail_positions = (tail_positions * s_sample_rate + startIdx)
                    ...
                    pred_scores.append(pred_scores.data.cpu().numpy())
            #FIXME: debug here!
            pred_segments = np.stack(pred_segments)
