#!/usr/bin/env python
#Update: copied from version gru2heads_mnist_v5_v7_EMD_update_mlayer.py
...
from PtrNet2.PointerGRU2Heads_v5 import PointerNetwork
from vsSummDevs.datasets import vsSumLoader3_c3dd
# from Exp_Synthetic2.Minist_Apperance.CreateMNIST_2Heads_online_v7 import MNIST
...
import PtrNet2.Metrics as Metrics
...
parser.add_argument('--dropout', default=0.75, type=float, help='dropout rate for training a network')
...
def main():
    ...
    save_directory = os.path.join(project_root, 'ckpts', '{:s}-{:s}-assgin{:.2f}-alpha{:.4f}-dim{:d}-dropout{:.4f}-seqlen{:d}-ckpt'.
                                  format(script_name_stem, args.dataset, hassign_thres, args.alpha, args.hidden_dim, args.dropout, args.seq_len))
    ...
    train_dataset = vsSumLoader3_c3dd.cDataset(dataset_name=args.dataset, split='train', seq_length=args.seq_len, overlap=0.9, sample_rate=[4, 8])
    val_dataset = vsSumLoader3_c3dd.cDataset(dataset_name=args.dataset, split='val', seq_length=args.seq_len, overlap=0.9, sample_rate=[4, 8])
