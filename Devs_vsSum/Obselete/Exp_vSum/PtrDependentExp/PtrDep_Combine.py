...
# update: for boyu to run.
# current version: Ptr is quite old and should be totally replaced.
import os
import sys
...
import argparse
from PtrNet.PointerNet2PointDepOffset import PointerNet #TODO: should be replaced with the newest version
...
import vsSummDevs.Exp_vSum.vsSum_Evaluator_MultiScale as Evaluator
...
parser.add_argument('--resume', '-r', default='/home/zwei/Dev/NetModules/Exp_vSum/PtrDependentExp/vsPtrDep_Combine/checkpoint_train_loss.pth.tar', type=str, help='resume from previous ')
...
def main():
    ...
    val_evaluator = Evaluator.Evaluator(dataset_name='SumMe', split='val', clip_size=100)
    ...
    train_dataset = LocalDataLoader.Dataset(dataset_name='SumMe', split='train', clip_size=args.clip_size, output_score=True, sample_rates=[1, 5, 10])
    ...
    for epoch in range(args.start_epoch, args.nof_epoch):
        ...
        for i_batch, sample_batched in enumerate(train_dataloader):
            ...
            overlap = loss_transforms.IoU_Overlaps(segment_indices.data, gt_index_batch.data)
        ...
        print(
            "Train -- Epoch :{:06d}, LR: {:.6f},\tloc-loss={:.4f}\tcls-loss={:.4f}\ttop1={:.4f}\ttop3={:.4f}\tF1_1={:.4f}\tF1_3={:.4f}".format(
                epoch,
                model_optim.param_groups[0]['lr'], loc_losses.avg, cls_losses.avg, Accuracy_Top1.avg,
                Accuracy_Top3.avg,
        ...
        }, isBest_status, file_direcotry='vsPtrDep_Combine')
