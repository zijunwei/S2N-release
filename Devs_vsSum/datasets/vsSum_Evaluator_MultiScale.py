...
class Evaluator():
    ...
    def Evaluate_get_segment_scores(self, model, use_cuda=True):
        F1s = 0
        n_notselected_seq = 0
        widgets = [' -- [ ',progressbar.Counter(), '|', str(self.dataset_size), ' ] ',
               progressbar.Bar(),  ' name:  ', progressbar.FormatLabel(''),
        ...
        for video_idx, (s_name, s_feature, s_groundtruth) in enumerate(zip(self.videonames, self.videofeatures, self.groundtruthscores)):
            ...
            s_F1, _, _, max_user_id = sum_tools.evaluate_summary(pred_framescores, s_groundtruth.transpose(), self.eval_metrics, return_user_id=True)

            s_instance_gt_pred = {}
            s_instance_gt_pred['name'] = s_name
            s_instance_gt_pred['gt_max_user'] = max_user_id
            s_instance_gt_pred['gt_user_summary'] = s_groundtruth[:, max_user_id]
            s_instance_gt_pred['pred_frame_01scores'] = pred_framescores
            pred_frame_raw_scores = np.zeros([n_frames])
            n_pred_seg = updated_segments.shape[0]
            for seg_id in range(n_pred_seg):
                pred_frame_raw_scores[updated_segments[seg_id,0]: updated_segments[seg_id,1]] = updated_scores[seg_id]
            s_instance_gt_pred['pred_frame_rawscores'] = pred_frame_raw_scores

            instances_gt_pred.append(s_instance_gt_pred)
        ...
        return F1s/self.dataset_size, instances_gt_pred
