...
class cDataset(data.Dataset):
    def __init__(self, dataset_name='TVSum', split='train', seq_length=90, overlap=0.9, sample_rate=None,
        ...
        for file_dix, s_filename in enumerate(self.filenames):
            ...
            s_features = Kydataset[Kykey]['features']
            ...
            s_n_users = s_scores.shape[1]
            n_users += s_n_users

            # s_segments, s_segment_scores = LoaderUtils.convertscores2segs(s_scores) # load segments, check this function...

            s_segments, s_segment_scores = LoaderUtils.convertscores2segs(s_scores)
            s_segments = rep_conversions.selecteTopSegments(s_segments, s_segment_scores, n_frames)
            ...
            for s_sample_rate in self.sample_rate:
                ...
                if s_seq_len <= n_frames:
                    ...
                    while start_idx < n_frames and isInbound:
                        ...
                        for s_action, s_score in zip(s_segments, s_segment_scores):
                            ...
                            if compute_intersection(s_action, s_instance_window) == 1:
                                ...
                                s_instance['s_scores'].append(s_score)
    ...
    def __getitem__(self, index):
        ...
        s_segment_scores = np.zeros(self.maximum_outputs, dtype=float)
        ...
        for idx in range(n_effective_idxes):
            ...
            s_segment_scores[idx] = s_instance['s_scores'][idx]
        ...
        s_segment_scores = torch.FloatTensor(s_segment_scores)
        # print(s_start_idxes)
        # print(n_effective_idxes)
        return s_clip_feature, s_start_idxes, s_end_idxes, n_effective_idxes, actionness_scores, s_segment_scores
...
if __name__ == '__main__':
    ...
    sDataset = cDataset(dataset_name='TVSum', split='val', seq_length=90, overlap=0.9, sample_rate=[4], rdOffset=True,
                        rdDrop=True)
