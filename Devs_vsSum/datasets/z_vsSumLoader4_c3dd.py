...
import os
import sys
#Update: compared to 3, only generate 1 annotation per video
...
class cDataset(data.Dataset):
    def __init__(self, dataset_name='SumMe', split='train', seq_length=90, overlap=0.9, sample_rate=None,
                 feature_file_ext='npy', rdOffset=False, rdDrop=False, train_val_perms=range(20), data_path='/home/zwei/datasets'):
        ...
        for file_dix, s_filename in enumerate(self.filenames):
            ...
            combined_summary = np.sum(s_usersummaries, axis=1, keepdims=True)
            combined_segments, combined_scores = LoaderUtils.convertscores2segs(combined_summary)
            combined_summary_segments =  rep_conversions.selecteTopSegments(combined_segments, combined_scores, n_frames)
            ...
            #     s_segments = LoaderUtils.convertlabels2segs(
            #         s_usersummaries[:, [s_user]])  # load segments, check this function...
            n_positive_instances += len(combined_summary_segments)
            for s_sample_rate in self.sample_rate:
                ...
                if s_seq_len <= n_frames:
                    ...
                    while start_idx < n_frames and isInbound:
                        ...
                        for s_action in combined_summary_segments:
