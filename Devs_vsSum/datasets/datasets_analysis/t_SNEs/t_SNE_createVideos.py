...
def scatter(x, scores, currentIdx, sample_rate=1):
    n_colors = 11
...
for video_idx, s_filename in enumerate(video_file_stem_names):
    ...
    s_save_dir = dir_utils.get_dir(os.path.join(save_dir, s_filename+'-NonNorm'))
    pbar= progressbar.ProgressBar(max_value=video_features.shape[0])
    for curIdx in range(video_features.shape[0]):
        pbar.update(curIdx)
        f, ax, _, _ = scatter(digits_proj, avg_labels, curIdx, sample_rate)
        # ax.title(s_filename)
        #
        plt.title(s_filename+'  {:05d}'.format(curIdx*sample_rate))
    ...
    ffmpeg_command = ["ffmpeg",  "-r", str(25), "-i", '"{:s}"'.format(os.path.join(s_save_dir, "%04d.{:s}".format('PNG'))),
                      "-vcodec", "mpeg4", "-q:v", "5", '"{:s}"'.format(os.path.join(save_dir, "{:s}-NonNorm.mp4".format(s_filename.replace(' ', '_'))))]
