...
for video_idx, s_filename in enumerate(video_file_stem_names):
    ...
    for i in range(len(avg_labels)):
        avg_labels[avg_indices[i]] = 1 if i <= endpoints else 0


    # threshold = avg_labels[avg_indices[int(0.15*len(avg_labels))]]
    # avg_labels[avg_labels > threshold] = 1
    # avg_labels[avg_labels <= threshold] = 0
    user_scores_list = [user_labels[:, i] for i in range(user_labels.shape[1])]
    s_F1_score = metrics.averaged_F1_score(y_trues=user_scores_list, y_score=avg_labels.tolist())

    print "[{:02d} | {:02d}]\t{:s}: \t{:.04f}\tlrato:{:.02f}".format(video_idx, len(video_file_stem_names), s_filename, s_F1_score, sum(avg_labels)*1./len(avg_labels))
    t_acc += (s_F1_score)
