#!/usr/bin/env python
#Update: same as s4 but with auxilary loss!
...
from SDN.PointerLSTM2Heads_v3 import PointerNetwork
from ActionLocalizationDevs.datasets.THUMOS14.dataloader_inception_4 import THUMOST14
from PtUtils import cuda_model
# from Losses import h_assign_fix
...
# parser.add_argument('--resume', '-r', default='/home/zwei/Dev/NetModules/PtrNet2/lstm2heads_proposal_s4_ckpt/checkpoint_{:04d}.pth.tar', type=str, help='resume from previous ')
...
def main():
    ...
    for epoch in range(args.start_epoch, args.nof_epoch+args.start_epoch):
            ...
            for i_batch, sample_batch in enumerate(train_dataloader):
                ...
                actionness_scores = Variable(sample_batch[4])
                ...
                if use_cuda:
                    ...
                    actionness_scores = actionness_scores.cuda()

                gt_positions = torch.stack([start_indices, end_indices], dim=-1)

                head_pointer_probs, head_positions, tail_pointer_probs, tail_positions, cls_scores, pred_actionness_scores = model(feature_batch)

                pred_actionness_diff = pred_actionness_scores[:,1:]-pred_actionness_scores[:,0:-1]
                actionness_diff = actionness_scores[:, 1:]

                pred_actionness_diff = pred_actionness_diff.contiguous().view(-1)
                actionness_diff = actionness_diff.contiguous().view(-1)

                aux_loss = 1 - pred_actionness_diff*actionness_diff
                aux_loss = aux_loss.clamp(min=0)
                ...
                total_loss = alpha * (prediction_head_loss + prediction_tail_loss) + cls_loss + alpha_aux*aux_loss
            ...
            'IoU': IOU.avg}, (epoch+1), file_direcotry='lstm2heads_proposal_s5_ckpt')
