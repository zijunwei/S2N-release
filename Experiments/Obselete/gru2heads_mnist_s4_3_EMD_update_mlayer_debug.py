...
from Exp_Synthetic2.Minist_Apperance.CreateMNIST_2Heads_online_v3 import MNIST
...
parser.add_argument('--resume', '-r', default='/home/zwei/Dev/NetModules/PtrNet2/gru2heads_mnist_s4-3_EMD_mlayer_ckpt/checkpoint_{:04d}.pth.tar', type=str, help='resume from previous ')
...
def main():
    ...
    save_directory = 'gru2heads_mnist_s4-3_EMD_AUG_mlayer_ckpt'
    ...
    for epoch in range(args.start_epoch, args.nof_epoch+args.start_epoch):
            ...
            for i_batch, sample_batch in enumerate(val_dataloader):
                ...
                assigned_scores, assigned_locations = h_assign.Assign_Batch_all_valid(gt_positions, pred_positions, thres=0.5)
            ...
            #
            #         assigned_head_positions = to_one_hot(assigned_head_positions, 100)
            #         assigned_tail_positions = to_one_hot(assigned_tail_positions, 100)
            #
            ...
            #     pred_positions = torch.stack([head_positions, tail_positions], dim=-1)
            #
            #     assigned_scores, assigned_locations = h_assign_h_new.Assign_Batch_all_valid(gt_positions, pred_positions,thres=0.5)
            #     # if np.sum(assigned_scores) > 1:
            #     #     print("DEBUG")
            #     # correct_predictions = np.sum(assigned_scores[:,:args.n_outputs])
            #     # cls_rate = correct_predictions*1./np.sum(assigned_scores)
