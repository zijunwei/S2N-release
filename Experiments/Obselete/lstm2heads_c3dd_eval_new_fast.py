...
from SDN.PointerGRU2Heads_v4_mlayer_3in import PointerNetwork
from PtUtils import cuda_model
from ActionLocalizationDevs.datasets.THUMOS14.single_video_evaluator_loader import SingleVideoLoader
import progressbar
import SDN.helper
...
parser.add_argument('--eval', type=str, default='/home/zwei/Dev/NetModules/PtrNet2/gru2heads_c3dd_s4_EMD_update_mlayer-assgin0.75-alpha0.1000-dim512-dropout0.7500-ckpt', help='check-point file')
parser.add_argument('--fileid', type=int, default=11, help='file id in ckpt file')
...
def main():
    ...
    proposal_save_file = 'Dev/NetModules/ActionLocalizationDevs/PropEval/baselines_results/{:s}-{:04d}_thumos14_test.csv'.format(savefile_stem, ckpt_idx)
    feature_directory = os.path.join(user_home_directory, 'datasets/THUMOS14/features/c3dd-fc7-red500')

    ground_truth_file = os.path.join(user_home_directory, '/home/zwei/Dev/NetModules/ActionLocalizationDevs/action_det_prep/thumos14_tag_test_proposal_list_c3dd.csv')
    ...
    for video_idx, s_target_filename in enumerate(target_file_names):
        ...
        print("[{:d} | {:d}]{:s}\t {:d} Frames\t {:d} Clips\t{:d} Proposals".format(video_idx, len(target_file_names), s_target_filename, n_video_len, n_video_clips, n_proposals))
