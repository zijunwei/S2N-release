...
parser.add_argument('--resume', '-r', default='/home/zwei/Dev/NetModules/PtrNet2/gru2heads_c3dd_s4_EMD_update_mlayer-assgin0.75-alpha0.1000-dim256-dropout0.5000-ckpt/checkpoint_{:04d}.pth.tar', type=str, help='resume from previous ')
...
#                 pred_positions = torch.stack([head_positions, tail_positions], dim=-1)
#
#                 assigned_scores, assigned_locations = h_assign_h_new.Assign_Batch(gt_positions, pred_positions, gt_valids, thres=hassign_thres)
#                 # if np.sum(assigned_scores) > 1:
#                 #     print("DEBUG")
#                 # correct_predictions = np.sum(assigned_scores[:,:args.n_outputs])
#                 # cls_rate = correct_predictions*1./np.sum(assigned_scores)
