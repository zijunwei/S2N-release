...
def main():
    ...
    save_directory = 'gru2heads_proposal_s4-3_EMD_AUG_ckpt-db'
    ...
    for epoch in range(args.start_epoch, args.nof_epoch+args.start_epoch):
            ...
            for i_batch, sample_batch in enumerate(train_dataloader):
                ...
                assigned_scores, assigned_locations = h_assign.Assign_Batch(gt_positions, pred_positions, valid_indices, thres=0.)
                ...
                if torch.sum(assigned_scores)>0:
                    ...
                    exact_loc_losses.update((prediction_head_loss.data.item() + prediction_tail_loss.data.item()), torch.sum(assigned_scores).item())

                    total_loss = alpha * (prediction_head_loss + prediction_tail_loss) + cls_loss
                else:
                    loc_losses.update(0., feature_batch.size(0))
                    total_loss = cls_loss
                ...
                # model_optim.step()
                total_losses.update(total_loss.data.item(), 1)


            print(
                "Train -- Epoch :{:06d}, LR: {:.6f},\tloss={:.4f}, \t c-loss:{:.4f}, \tavg-loc-loss:{:.4f}\texact-cls-loss:{:.4f}\tloc-Avg-IOU:{:.4f}".format(
                    epoch,
                    model_optim.param_groups[0]['lr'], total_losses.avg, cls_losses.avg, loc_losses.avg, exact_loc_losses.avg,
                    IOU.avg))
            ...
            print(
                "Val -- Epoch :{:06d}, LR: {:.6f},\tloss={:.4f}, \t c-loss:{:.4f}, \tavg-loc-loss:{:.4f}\texact-cls-loss:{:.4f}\tloc-Avg-IOU:{:.4f}".format(
                    epoch,
                    model_optim.param_groups[0]['lr'], total_losses.avg, cls_losses.avg, loc_losses.avg, exact_loc_losses.avg,
                    IOU.avg))
