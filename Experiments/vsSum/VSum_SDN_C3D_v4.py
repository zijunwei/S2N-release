...
def main():
    ...
    val_dataset = vsSumLoader3_c3dd.cDataset(dataset_name=args.dataset, split='val', seq_length=args.seq_len, overlap=0.9, sample_rate=[args.sample_rate],
    ...
    for epoch in range(args.start_epoch, args.nof_epoch+args.start_epoch):
            ...
            logger.info(
                "Val -- Epoch :{:06d},\tIOU:{:.4f}, \tF1s{:.4f}".format(
                    epoch, matched_IOU.avg, F1s))
