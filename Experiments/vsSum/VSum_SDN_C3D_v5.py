# similar to v4, but use 17 train, 2 val, 5 test
...
def main():
    ...
    train_val_perms = np.delete(train_val_test_perms, args.split, 0).reshape([-1])
    ...
    test_evaluator = Evaluator.Evaluator(dataset_name=args.dataset, split='test', seq_length=args.seq_len, overlap=0.9, sample_rate=[args.sample_rate],
                sum_budget=0.15, train_val_perms=test_perms, eval_metrics=args.eval_metrics, data_path=data_path)
    ...
    for epoch in range(args.start_epoch, args.nof_epoch+args.start_epoch):
            ...
            'IoU': IOU.avg,
            'val_F1s': val_F1s,
            'test_F1s': test_F1s}, (epoch+1), file_direcotry=save_directory)
