...
from Devs_vsSum.datasets import vsTVSumLoader3_googlenet
from Devs_vsSum.datasets import vsTVSum_Evaluator_MultiScale_googlenet as Evaluator
from PtUtils import cuda_model
from Losses import h_assign as h_match
from Losses import f_assign as f_match

from Losses.losses import EMD_L2, to_one_hot, ClsLocLoss2_OneClsRegression_v2
import progressbar
...
def main():
    ...
    save_directory = dir_utils.get_dir(os.path.join(project_root, 'ckpts', '{:s}-{:s}-{:s}-split-{:d}-claweight-{:s}-{:.1f}-assgin{:.2f}-alpha{:.4f}-dim{:d}-dropout{:.4f}-seqlen{:d}-samplerate-{:d}-{:s}-{:s}'.
                                  format(script_name_stem, args.dataset, args.eval_metrics, args.split, args.set_cls_weight, args.cls_pos_weight, args.hassign_thres, args.alpha, args.hidden_dim, args.dropout, args.seq_len, args.sample_rate, loss_type[args.EMD], match_type[args.hmatch])))
    ...
    model = PointerNetwork(input_dim=args.input_dim, embedding_dim=args.embedding_dim,
                           hidden_dim=args.hidden_dim, max_decoding_len=args.net_outputs, dropout=args.dropout, n_enc_layers=2, output_classes=1)
    ...
    train_dataset = vsTVSumLoader3_googlenet.cDataset(dataset_name=args.dataset, split='train', seq_length=args.seq_len, overlap=0.9, sample_rate=[args.sample_rate],
    ...
    for epoch in range(args.start_epoch, args.nof_epoch+args.start_epoch):
            ...
            for i_batch, sample_batch in enumerate(train_dataloader):
                ...
                gt_scores = Variable(sample_batch[5])
                ...
                head_pointer_probs, head_positions, tail_pointer_probs, tail_positions, cls_scores, _ = model(feature_batch, decode_len=train_dataset.maximum_outputs)
                ...
                if args.hmatch:
                    ...
                    assigned_scores, assigned_locations, assigned_overlaps, total_valid, total_iou = f_match.Assign_Batch_v2_repression(gt_positions, pred_positions, gt_valids, gt_scores, thres=args.hassign_thres)
                ...
                pred_scores = F.hardtanh(cls_scores, min_val=0, max_val=1).contiguous().view(-1)
                pred_scores = pred_scores.contiguous().view(-1)
                assigned_scores = assigned_scores.contiguous().view(-1)
                assigned_overlaps = assigned_overlaps.contiguous().view(-1)
                cls_loss = ClsLocLoss2_OneClsRegression_v2(pred_scores, assigned_overlaps)
