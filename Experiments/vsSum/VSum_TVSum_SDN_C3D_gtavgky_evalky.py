# at training: ground truth is by taking average of each annotation, use DP to generate a new ground truth
# at evaluation: use KY user_summary
...
from Devs_vsSum.datasets import vsTVSum_Loader3_c3dd_avgky
from Devs_vsSum.datasets import vsTVSum_Evaluator_MultiScale_ky as Evaluator
...
parser.add_argument('--sample_rate', default=16, type=int, help='sample rate')
...
def main():
    ...
    train_dataset = vsTVSum_Loader3_c3dd_avgky.cDataset(dataset_name=args.dataset, split='train', seq_length=args.seq_len, overlap=0.9, sample_rate=[args.sample_rate],
                train_val_perms=train_perms, data_path=data_path)
    val_dataset = vsTVSum_Loader3_c3dd_avgky.cDataset(dataset_name=args.dataset, split='val', seq_length=args.seq_len, overlap=0.9, sample_rate=[args.sample_rate],
                train_val_perms=val_perms, data_path=data_path)
    train_evaluator = Evaluator.Evaluator(dataset_name=args.dataset, split='tr', seq_length=args.seq_len, overlap=0.9, sample_rate=[args.sample_rate],
    ...
    for epoch in range(args.start_epoch, args.nof_epoch+args.start_epoch):
            ...
            logger.info(
                "Val -- Epoch :{:06d},\tloc-Avg-matched_IOU:{:.4f},\tF1s{:.4f}".format(
                    epoch, matched_IOU.avg, val_F1s))
