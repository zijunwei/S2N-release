# visualize the selected segment on validation set
...
parser.add_argument('--nof_epoch', default=30, type=int, help='Number of epochs')
parser.add_argument('--lr', type=float, default=0.0001, help='Learning rate')
parser.add_argument('--eval', '-e', default='y', type=str2bool, help='evaluate only')
parser.add_argument('--eval_epoch', default=9, type=int, help='which epoch to use for evaluate')
# GPU
...
parser.add_argument('--dataset', default='SumMe', type=str, help='dataset name')
...
def main():
    ...
    script_name_stem = 'VSum_SDN_C3D_v4'
    ...
    log_file = os.path.join(save_directory, 'log-{:s}_eval.txt'.format(dir_utils.get_date_str()))
    ...
    if args.eval:
        ...
        ckpt_filename = os.path.join(save_directory, 'checkpoint_{:04d}.pth.tar'.format(args.eval_epoch+1))
        print(ckpt_filename)
        ...
        logger.info("=> loading checkpoint '{}', current val f1s: {:.04f}".format(ckpt_filename, val_f1s))
    ...
    model.eval()
    F1s, instance_gt_pred = val_evaluator.Evaluate_get_segment_scores(model)

    logger.info(
        "Val -- Epoch :{:06d}, \tF1s:{:.4f}".format(
            args.eval_epoch, F1s))

    for s_instance_gt_pred in instance_gt_pred:
        plot_gt_pred(s_instance_gt_pred['gt_user_summary'], s_instance_gt_pred['pred_frame_01scores'],
                    s_instance_gt_pred['pred_frame_rawscores'], s_instance_gt_pred['name'], save_directory)
        to_save = dict()
        to_save['gt_user_summary'] = s_instance_gt_pred['gt_user_summary']
        to_save['pred_frame_01scores'] = s_instance_gt_pred['pred_frame_01scores']
        sio.savemat(os.path.join(save_directory, s_instance_gt_pred['name']+'.mat'), to_save)


def plot_gt_pred(gt, pred_01scores, pred_rawscores, video_name, save_directory):
    fig = plt.figure()
    ax = fig.add_subplot(111)
    n_frames = gt.shape[0]
    ax.plot(np.arange(0, n_frames), gt, label='gt')
    ax.plot(np.arange(0, n_frames), pred_01scores, label='pred')
    # ax.plot(np.arange(0, n_frames), pred_rawscores, label='pred_raw')
    plt.legend()
    plt.savefig(os.path.join(save_directory, '%s.pdf'%(video_name)), dpi=300)
    plt.close(fig)
