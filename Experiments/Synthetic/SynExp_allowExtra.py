...
from SDN.PointerGRU2Heads_v7 import PointerNetwork
from Devs_SyntheticV2.SDN_MNIST_s1_1_online import cDataset
...
parser.add_argument('--dataset', default='SumMe', type=str, help='Specify a dataset')
...
parser.add_argument('--sufix', default='detach', type=str, help='suffix')
...
# script_specific:
parser.add_argument('--extra', default=3, type=int, help='extra digits of prediction allowed')
...
def main():
    ...
    if args.resume is None:
        ...
        save_directory = dir_utils.get_dir(os.path.join(project_root, 'ckpts', '{:s}'.format(args.dataset), '{:s}-{:s}-assgin{:.2f}-alpha{:.4f}-dim{:d}-dropout{:.4f}-seqlen{:d}-{:s}-{:s}-extra{:d}'.
                                      format(script_name_stem, args.sufix, args.hassign_thres, args.alpha, args.hidden_dim, args.dropout, args.seq_len, loss_type[args.EMD], match_type[args.hmatch], args.extra)))
    else:
    ...
    for epoch in range(args.start_epoch, args.nof_epoch+args.start_epoch):
            ...
            for i_batch, sample_batch in enumerate(val_dataloader):
                ...
                matched_valid, matched_iou = h_match.totalMatch_Batch_extra(gt_positions, pred_positions, gt_valids, allowed_extra=args.extra)
                if matched_valid>0:
