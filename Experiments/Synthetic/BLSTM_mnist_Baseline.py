...
from Devs_SyntheticV2.SDN_MNIST_s1_3output_LSTM_online import MNIST #Update: output top 3!
...
import numpy as np
import Losses.h_assign as h_assign
...
def main():
    ...
    for epoch in range(args.start_epoch, args.nof_epoch+args.start_epoch):
            ...
            for i_batch, sample_batch in enumerate(train_dataloader):
                ...
                s_valid_indices = sample_batch[-1]
                starting_indices = sample_batch[2]
                ending_indices = sample_batch[3]
                ...
                gt_positions = torch.stack([starting_indices, ending_indices], dim=-1)

                for s_instance_idx in range(feature_batch.shape[0]):

                    pred_labels_starting = pred_labels[s_instance_idx, :, 1].data.cpu().numpy()
                    pred_labels_ending = pred_labels[s_instance_idx, :, 2].data.cpu().numpy()
                    top_pick = s_valid_indices[s_instance_idx, 0].item()
                    topK_start = np.argsort(pred_labels_starting)[::-1][:top_pick]
                    topK_end = np.argsort(pred_labels_ending[::-1][:top_pick])
                    # s_labels = labels[s_instance_idx].data.cpu().numpy()
                    # predict_vector = np.zeros_like(s_labels)
                    topK_start = np.sort(topK_start)
                    topK_end  = np.sort(topK_end)
                    predicted_positions = np.stack((topK_start, topK_end)).transpose()
                    total_valid, total_iou = h_assign.totalMatch(gt_positions[s_instance_idx,:top_pick,:], predicted_positions, gt_valid=top_pick)
                    # for count in range(top_pick):
                    #     predict_vector[topK_start[count]:topK_end[count]]= 1

                    # iou = np.sum(predict_vector == s_labels).item() * 1.
