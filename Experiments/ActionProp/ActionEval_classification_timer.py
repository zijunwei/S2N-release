...
parser.add_argument('--eval', type=str, default='/home/zwei/Dev/NetModules/ckpts/THUMOS_final/ActionExp_v3-NoDepsIn3-assgin0.50-alpha0.1000-dim512-dropout0.5000-seqlen90-EMD-HUG', help='check-point file')
...
def main():
    ...
    proposal_save_file = 'Dev/NetModules/Devs_ActionProp/PropEval/baselines_results/{:s}-{:04d}-timer_thumos14_test_rebuttal.csv'.format(savefile_stem, ckpt_idx)
    ...
    for video_idx, s_target_filename in enumerate(target_file_names):
        ...
        for batch_idx, data in enumerate(singlevideo_dataset):
            ...
            #    s_run_time =  time.time() - s_run_timer_start
            #    print("time for single run: {:.4f}\n".format(s_run_time))
        ...
        print("[{:d} | {:d}]{:s}\t {:d} Frames\t {:d} Clips\t{:d} Proposals, Non-repeat:{:d}, time:{:f}".
              format(video_idx, len(target_file_names), s_target_filename, n_video_len, n_video_clips, n_proposals, nms_predictions.shape[0], end_time-start_time))
