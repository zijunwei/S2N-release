...
parser.add_argument('--sufix', default='Release', type=str, help='suffix')
...
def main():
    ...
    if args.resume is None:
        ...
        save_directory = dir_utils.get_dir(os.path.join(project_root, 'ckpts', '{:s}'.format(args.dataset), '{:s}-{:s}-assgin{:.2f}-alpha{:.4f}-dim{:d}-dropout{:.2f}-seqlen{:d}-{:s}-{:s}'.
    ...
    widgets = ['Train: ', ' -- [ ', progressbar.Counter(), '|', str(len(train_dataloader)), ' ] ',
