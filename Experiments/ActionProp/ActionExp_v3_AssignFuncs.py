#Update: Compared to V3, this provides an experiment on reducing the assingment functions to simpler forms
#
import os, sys
...
parser.add_argument('--assign_method', default='full', help='h-assign types: [ full ] | rank | greedy')
...
def main():
    ...
    if args.resume is None:
        ...
        save_directory = dir_utils.get_dir(os.path.join(project_root, 'ckpts', '{:s}'.format(args.dataset), '{:s}-{:s}-assgin{:.2f}-alpha{:.4f}-dim{:d}-dropout{:.4f}-seqlen{:d}-{:s}-{:s}-{:s}'.
                                      format(script_name_stem, args.sufix, args.hassign_thres, args.alpha, args.hidden_dim, args.dropout, args.seq_len, loss_type[args.EMD], match_type[args.hmatch], args.assign_method)))
    else:
    ...
    for epoch in range(args.start_epoch, args.nof_epoch+args.start_epoch):
            ...
            for i_batch, sample_batch in enumerate(train_dataloader):
                ...
                if args.hmatch:
                    assigned_scores, assigned_locations, total_valid, total_iou = h_match.Assign_Batch_v3(gt_positions, pred_positions, gt_valids, thres=args.hassign_thres, assign_type=args.assgin_method)
                else:
                    #FIXME: do it later!
