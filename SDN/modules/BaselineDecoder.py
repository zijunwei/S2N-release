...
#Update: compared to v4, this is just a baseline decoder
class Decoder(nn.Module):
    def __init__(self,
        ...
        #FIXME: this is redudant
        self.cls = torch.nn.Conv1d(hidden_dim, 2, kernel_size=3, stride=1, padding=0)
    ...
    def forward(self, decoder_input, embedded_inputs, hidden, context, max_len=None):
        ...
        #FIXME: this recurrance is not based on inputs...
        def recurrence(x, hidden, prev_e_enc):
            ...
            _, idxs_e = self.decode_argmax(logits_e, context)
            ...
            _, idxs_s = self.decode_argmax(logits_s,context)

            feature = torch.stack([g_l, g_l, g_l], dim=-1)
            #
