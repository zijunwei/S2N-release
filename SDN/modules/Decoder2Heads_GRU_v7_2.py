...
class Decoder(nn.Module):
    def __init__(self,
        ...
        # self.score_conv = torch.nn.Conv1d(hidden_dim, hidden_dim/2, kernel_size=3, stride=1, padding=0)
        #Update: only here the name is different, to compensate for the running in ActionExp_v4
        self.score_linear1 = torch.nn.Linear(hidden_dim, hidden_dim / 2)

        self.score_linear2 = torch.nn.Linear(hidden_dim / 2, outputclasses)
    ...
    def forward(self, decoder_input, embedded_inputs, hidden, context, max_len=None):
        ...
        def recurrence(x, hidden):
            ...
            # s_encoding = g_l_e + g_l

            _, logits_s = self.pointer_s(g_l, context)
            ...
            score_intermediate = F.dropout(F.relu(self.score_linear1(g_l)), 0.5)

            cls_score = self.score_linear2(score_intermediate.view(score_intermediate.size(0), -1))


            # next_input = g_l_e - g_l_s
            next_input = None
        ...
        for i in steps:
            hx, _, probs_s, probs_e, idxs_head, idxs_tail, cls_score = \
