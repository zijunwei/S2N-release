...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, dropout=0, useGRU=False):
        super(Encoder, self).__init__()
        self.hidden_dim = hidden_dim
        #FIXME: now there is only 1 layer (double direction, dropout will be 1)
        if useGRU:
            self.rnn=nn.GRU(input_dim, hidden_dim, dropout=0., bidirectional=True)
        else:
            self.rnn = nn.LSTM(input_dim, hidden_dim, dropout=0., bidirectional=True)
