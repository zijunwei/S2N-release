...
def main():
    ...
    model = NERModel.CNERPointer(char_dim=args.char_dim, seg_dim=args.seg_dim, hidden_dim=args.hidden_dim, max_length=15,
                                 output_classes=4, dropout=args.dropout, embedding_path=embedding_file, id_to_word=id_to_char, easy_load=embedding_easy_file)
    ...
    for epoch in range(args.start_epoch, args.nof_epoch+args.start_epoch):
        ...
        for batch_idx, batch in enumerate(train_manager.iter_batch(shuffle=True)):
            ...
            assigned_scores, assigned_locations = h_assign.Assign_Batch_MClass(gt_positions, gt_classes, pred_positions, gt_valids,
                                                                               thres=args.hassign_thres)
            #UPDATE
            if np.sum(assigned_scores)>=1:
                iou_rate, effective_positives = Metrics.get_avg_iou3(np.reshape(pred_positions.data.cpu().numpy(), (-1, 2)),
            ...
            if torch.sum(assigned_scores)>0:
                ...
                assigned_head_positions = torch.masked_select(assigned_head_positions, (assigned_scores>0).byte())
                assigned_tail_positions = torch.masked_select(assigned_tail_positions, (assigned_scores>0).byte())
        ...
        for batch_idx, batch in enumerate(dev_manager.iter_batch(shuffle=False)):
            pbar.update(batch_idx)
            word_vectors = torch.LongTensor(batch[1])
            seg_vectors = torch.LongTensor(batch[2])
            ...
            assigned_scores, assigned_locations = h_assign.Assign_Batch_MClass(gt_positions, gt_classes,
                                                                               pred_positions, gt_valids,
                                                                               thres=args.hassign_thres)
            # UPDATE
            if np.sum(assigned_scores) >= 1:
                iou_rate, effective_positives = Metrics.get_avg_iou3(
