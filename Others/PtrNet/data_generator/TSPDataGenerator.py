...
class TSPDataset(Dataset):
    ...
    def _generate_data(self):
        ...
        points_list = []
        solutions = []
        pbar = progressbar.ProgressBar(max_value=self.data_size)
        # data_iter = tqdm(range(self.data_size), unit='data')
        print("Generating TSP points with Length {:d}\n".format(self.seq_len))
        ...
        pbar = progressbar.ProgressBar(max_value=self.data_size)
        if self.solve:
            for i, s_points in enumerate(points_list):
                pbar.update(i)
                solutions.append(self.solver(s_points))
        else:
            solutions = None
