...
# after a long time tweaking, it is still only possible to straighten a net specifically
def recursive_layer(model, level_expand=1):
    module_dict = collections.OrderedDict()
    for key, module in model._modules.items():
        if isinstance(module, torch.nn.Sequential) and level_expand>0:
                sub_dict = recursive_layer(module, level_expand-1)
                for s_key, s_item in sub_dict.items():
                    s_name = '{:s}.{:s}'.format(key, s_key)
                    module_dict[s_name] = s_item
        else:
...
class StraightenModel(nn.Module):
    def __init__(self, orig_model, level_expand=1, get_outputs=False):
        super(StraightenModel, self).__init__()
        self.nodes = recursive_layer(orig_model, level_expand)
    ...
    def forward(self, x):
        ...
        for key, node in self.nodes.items():
            if key == 'classifier.0':
                x = x.view(x.size(0), -1)
            x = node(x)
            name = '{:s}.{:s}'.format(key,node.__class__.__name__)
            if self.outputs:
                outputs[name] = x
        if self.outputs:
            return x, outputs
...
def main():
    ...
    print("Number of Params in {:s}\t{:d}".format(model_name, sum([p.data.nelement() for p in orig_model_vgg.parameters()])))
    model = cuda_model.convertModel2Cuda(straighten_model, args.gpu_id, args.multiGpu)
    model.eval()
