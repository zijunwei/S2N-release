...
import glob
from Devs_vsSum.datasets.TVSum import path_vars as TVSum_pathvars
from Devs_vsSum.datasets.SumMe import path_vars as SumMe_pathvars
from Devs_vsSum.datasets import ImageDirectoryDataset
...
paths = {'tvsum': TVSum_pathvars, 'summe':SumMe_pathvars}
...
def main():
    ...
    path_vars = paths[args.dataset.lower()]

    image_root_directory = os.path.join(path_vars.dataset_dir, 'frames')
    save_root_directory = dir_utils.get_dir(os.path.join(path_vars.dataset_dir, 'features/ImageNet/{:s}'.format(model_name)))
    ...
    for idx_dir, s_image_direcotry in enumerate(image_directories):
        ...
        s_dataset = ImageDirectoryDataset.ImageDirectoryDataset(s_image_direcotry, transform=val_transform)
