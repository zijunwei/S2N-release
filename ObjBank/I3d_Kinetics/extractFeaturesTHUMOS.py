...
parser.add_argument('--framedir', default='/nfs/bigeye/yangwang/DataSets/THUMOS14/frame', help='directory for frames')
parser.add_argument('--savedir', default='/home/zwei/datasets/THUMOS14/features/I3D', help='directory for saving features')
parser.add_argument('--startIdx', default=0, type=int, help='Starting Index')
parser.add_argument('--endIdx', default=None, type=int, help='Ending Index')
...
def main():
    ...
    save_root_directory = dir_utils.get_dir(args.savedir)
    chunk_size = 64
    image_directories = glob.glob(os.path.join(args.framedir, '*/'))
    startIdx = args.startIdx
    endIdx = args.endIdx or len(image_directories)
    for idx_dir, s_image_direcotry in enumerate(image_directories):
        if idx_dir<startIdx or idx_dir>endIdx:
            continue
