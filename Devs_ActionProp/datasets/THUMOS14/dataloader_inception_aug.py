...
class THUMOST14(data.Dataset):
    ...
    def __init__(self, seq_length=90, overlap=0.9, sample_rate=None, dataset_split='val', feature_file_ext='npy', rdOffset=False, rdDrop=False):
        self.feature_directory = os.path.join(user_root, 'datasets/THUMOS14/features/BNInception')
        self.feature_file_ext = feature_file_ext
        self.data_split = dataset_split
        if dataset_split == 'train' or dataset_split=='val':
            self.annotation_file = os.path.join(user_root, 'Dev/NetModules/ActionLocalizationDevs/action_det_prep/thumos14_tag_val_proposal_list.csv')
        elif dataset_split == 'test':
            self.annotation_file = os.path.join(user_root, 'Dev/NetModules/ActionLocalizationDevs/action_det_prep/thumos14_tag_test_proposal_list.csv')
        else:
...
if __name__ == '__main__':
    ...
    thumos_dataset = THUMOST14(seq_length=90, overlap=0.9, sample_rate=[1, 2, 4], dataset_split='val', rdOffset=True, rdDrop=True)
