...
def compute_intersection(action, window):
    ...
    intersection = np.maximum(y2 - y1, 0)
    intersection_rate = intersection*1./(action[1]-action[0])
...
miss_name = ('video_test_0001292','video_validation_0000190') # reasons for exclusion: 1292 has no positives, 190 is too short
...
class THUMOST14(data.Dataset):
    ...
    def __init__(self, seq_length=360, overlap=0.9, sample_rate=1, dataset_split='val', feature_file_ext='npy'):
        ...
        self.feature_directory = os.path.join(user_home_directory, 'datasets/THUMOS14/features/c3d-dense')
        self.feature_file_ext = feature_file_ext
        self.annotation_file = os.path.join(user_home_directory, 'Dev/NetModules/ActionLocalizationDevs/action_det_prep/thumos14_tag_{:s}_proposal_list.csv'.format(dataset_split))
        ...
        for s_movie_name in self.movie_instances.keys():
            ...
            while start_idx < n_frames and isInbound:
                ...
                for s_action in s_movie_instance:
                    ...
                    if compute_intersection(s_action, s_instance_window)>0:
