...
class THUMOST14(data.Dataset):
    ...
    def __init__(self, seq_length=360, overlap=0.9, sample_rate=1, dataset_split='val', feature_file_ext='npy'):
        ...
        self.annotation_file = '/home/zwei/Dev/NetModules/ActionLocalizationDevs/action_det_prep/thumos14_tag_{:s}_proposal_list.csv'.format(dataset_split)
        print("Reading training data list from {:s}\t clip len:{:d}, sample rate: {:d}".format(self.annotation_file, seq_length, sample_rate))
...
if __name__ == '__main__':
    ...
    thumos_dataset = THUMOST14(seq_length=360, overlap=0.9, sample_rate=4)
    start_location_distributions = np.zeros(360/4,dtype=int)
    end_location_distributions = np.zeros(360/4,dtype=int)
    for s_instance in thumos_dataset.instances:
        instance_actions = s_instance['actions']
        instance_start = s_instance['start']
        for s_action in instance_actions:
            rel_action_start = (s_action[0]-instance_start)/4
            start_location_distributions[rel_action_start] += 1
            rel_action_end = (s_action[1]-instance_start)/4
            end_location_distributions[rel_action_end] += 1
    print("DB")
