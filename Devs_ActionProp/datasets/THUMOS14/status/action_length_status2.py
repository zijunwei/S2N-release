...
np.random.seed(0)
random.seed(0)
# compared to v1, this version handles the imbalances in video lengths...
...
class THUMOST14(data.Dataset):
    ...
    def __init__(self, seq_length=90, overlap=0.9, sample_rate=None, dataset_split='val', feature_file_ext='npy', rdOffset=False, rdDrop=False):
        ...
        if dataset_split == 'train':
            selected_perms = train_selected_perms[:160]
    ...
    def __getitem__(self, index):
        ...
        for idx in range(n_effective_idxes):
            ...
            assert s_start_idxes[idx] >= 0 and s_start_idxes[idx]<self.seq_len, '{:d} start wrong!'.format(s_start_idxes[idx])
...
if __name__ == '__main__':
    dataset_split = 'test'
    if dataset_split == 'train':
        annotation_file = os.path.join(user_root,
                                            'Dev/NetModules/ActionLocalizationDevs/action_det_prep/thumos14_tag_val_proposal_list_c3dd.csv')
    elif dataset_split == 'test':
        annotation_file = os.path.join(user_root,
                                            'Dev/NetModules/ActionLocalizationDevs/action_det_prep/thumos14_tag_test_proposal_list_c3dd.csv')
    else:
        print("Cannot recognize split: {:s}".format(dataset_split))
        sys.exit(-1)
    ...
    ground_truth = pd.read_csv(annotation_file, sep=' ')
    n_ground_truths = len(ground_truth)

    segment_lengths = ground_truth[['f-init', 'f-end']].drop_duplicates().values
    segment_sizes = segment_lengths[:, 1] - segment_lengths[:, 0]
    segment_sizes[segment_sizes>500]=500
    bin_width = 1
    plt.hist(segment_sizes, bins=100)
    plt.title('{:s}'.format(dataset_split))
    plt.xlabel('frames')
    ...
    plt.tight_layout()
    plt.savefig('/home/zwei/datasets/THUMOS14/visualizations/action_len_dist/{:s}.pdf'.format(dataset_split))
