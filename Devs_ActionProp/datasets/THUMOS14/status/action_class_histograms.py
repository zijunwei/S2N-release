...
if __name__ == '__main__':
    ...
    for s_classname, class_id in zip(classnames,classids):
        ...
        bin_width = 1
        plt.hist(np.asarray(segment_lengths), bins=range(0, 30, bin_width))
        plt.title('{:s}-{:d}-Max:{:.2f}-Min:{:.2f}'.format(s_classname, len(segment_lengths),
                                                           max(segment_lengths), min(segment_lengths)))
        plt.xlabel('seconds')
        ...
        plt.tight_layout()
        plt.savefig('/home/zwei/datasets/THUMOS14/visualizations/action_len_dist/{:s}.pdf'.format(s_classname))
