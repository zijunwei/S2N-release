...
def process_proposal_list(norm_proposal_list, out_list_name, frame_dict):
    ...
    for idx, prop in enumerate(norm_proposals):
        vid = prop[0]
        if len(prop[2]) ==0:
            print('No GT in {:s}'.format(prop[0]))
            continue
        if not vid in frame_dict:
            print("{:s} not in frame dict".format(vid))
            continue
...
def z_process_proposal_list(norm_proposal_list, out_list_name, frame_dict):
    norm_proposals = load_proposal_file(norm_proposal_list)
    data_frame = []
    # processed_proposal_list = []
    for idx, prop in enumerate(norm_proposals):
        ...
        for s_gt in gt:
            ...
            label_idx = s_gt[0]
            data_frame.append([end, start, label_idx, frame_cnt, vid])


    results = pd.DataFrame(data_frame, columns=['f-end', 'f-init', 'label-idx', 'video-frames', 'video-name'])
    results.to_csv(out_list_name, sep=' ', index=False)
    # open(out_list_name, 'w').writelines(processed_proposal_list)


def parse_directory(path, key_func=lambda x: x[-11:],
                    rgb_prefix='i_', flow_x_prefix='x_', flow_y_prefix='y_'):
    """
...
def parse_c3ddfiles(path):
    ...
    frame_directories = glob.glob(os.path.join('/home/zwei/datasets/THUMOS14/frame', '*/'))
